# Script para configurar rutas de API Gateway
$ApiId = "zwxrsuw8o9"
$Region = "eu-west-1"
...
if (-not $identity) {
    Write-Host "Error: Credenciales AWS no validas" -ForegroundColor Red
    exit 1
}
...
# Funcion para crear metodo
function Add-Method {
    param($ResourceId, $Method, $Path)
    
    Write-Host "Configurando metodo $Method para $Path..." -ForegroundColor Cyan
    ...
    Write-Host "Metodo $Method configurado para $Path" -ForegroundColor Green
}

# Configurar metodos para el recurso principal de documentos
